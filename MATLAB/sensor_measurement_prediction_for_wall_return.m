...
if (theta_wrt_altitude < 0 || altitude_FEET < 0)
    ...
    if (table(yindex,xindex) == 99 || table(yindex,xindex+1) == 99 || table(yindex+1,xindex) == 99 || ...
            table(yindex+1,xindex+1) == 99)
        interpolated_value = 99;
        ...
        if (no_error_found_altitude_FEET == 1 && no_error_found_theta_wrt_altitude == 1)
            ...
            fx_y2 = ((x2-x_interest)/(x2-x1))*table(yindex+1,xindex)+ (x_interest-x1)/(x2-x1)*...
                table(yindex+1,xindex+1);
