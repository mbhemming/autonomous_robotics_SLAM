...
% the the current trejectory could hit the enviorment boundries
    ...
    if dir_x_component > 0+epsilon;
        x_min_search=x_robot;
        x_max_search=x_board_top_corner;
    elseif dir_x_component < 0-epsilon
        x_min_search=0;
        x_max_search=x_robot;
    else
        x_min_search=x_robot-epsilon;
        x_max_search=x_robot+epsilon;
    end
    
    if dir_y_component > 0+epsilon
        y_min_search=y_robot;
        y_max_search=y_board_top_corner;
    elseif dir_y_component < 0-epsilon
        y_min_search=0;
        y_max_search=y_robot;
    else
        y_min_search=y_robot-epsilon;
        y_max_search=y_robot+epsilon;
    end
